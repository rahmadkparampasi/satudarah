<table id="{{$IdForm}}dT" class=" display table align-items-centertable-striped table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalAddSegkec" onclick="addFill('seg_nmSegkec', '{{$tk['seg_nm']}}'); addFill('segkec_seg', '{{$tk['seg_id']}}'); $('#modalAddSegkecF').attr('data-div', '{{$IdForm}}data')"><i class="fas fa-plus"></i></button><br />
                @if (isset($tk['Segkec']))
            ...
            <td class="text-wrap">
                <button type="button" class="btn btn-warning" onclick="showForm('{{$IdForm}}card', 'block'); cActForm('{{$IdForm}}', '{{route('seg.update')}}'); addFill('seg_id', '{{$tk['seg_id']}}'); addFill('seg_nm', '{{$tk['seg_nm']}}');"><i class="fas fa-pen"></i></button>
