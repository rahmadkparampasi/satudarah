...
class SegController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->data['Seg'] = SegModel::select(['seg_id', 'seg_nm', 'seg_act', 'seg_kd'])->orderBy('seg_ord', 'desc')->get();
    ...
    public function insertData(Request $request)
    {
        ...
        $this->data['Pgn'] = $this->getUser();
        $Seg = SegModel::where('seg_kd', $request->seg_kd)->select(['seg_id'])->orderBy('seg_ord', 'desc')->get();
        if (count($Seg)>0) {
            $data['response'] = ['status' => 500, 'response' => 'error','type' => "danger", 'message' => 'Kode Segmentasi Daerah Sudah Ada'];
        }else{
            $SegModel->seg_nm = $request->seg_nm;
            $SegModel->seg_kd = $request->seg_kd;
    ...
    public function updateData(Request $request)
    {
        ...
        $Seg = DB::table('seg')->whereNotIn('seg_id', [$seg_id])->where('seg_kd', $request->seg_kd)->select()->get()->toArray();
        if ($Seg!=null) {
            $data['response'] = ['status' => 500, 'response' => 'error','type' => "danger", 'message' => 'Kode Segmentasi Daerah Sudah Ada'];
        }else{
            $update = $SegModel::where('seg_id', $seg_id)->update([
                'seg_nm' => $request->seg_nm,
                'seg_kd' => $request->seg_kd,
